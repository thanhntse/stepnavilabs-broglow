...
export class SkinProfileService {
  constructor(
    @InjectModel(SkinQuestion.name) private skinQuestionModel: Model<SkinQuestion>,
    @InjectModel(SkinProfile.name) private skinProfileModel: Model<SkinProfile>,
  ) {}

  // Skin Question Methods
  async createQuestion(createSkinQuestionDto: CreateSkinQuestionDto): Promise<SkinQuestion> {
    const createdQuestion = new this.skinQuestionModel(createSkinQuestionDto);
    return createdQuestion.save();
  }

  async findAllQuestions() {
    return this.skinQuestionModel.find({ isActive: true }).sort({ order: 1 }).exec();
  }
  ...
  async updateQuestion(id: string, updateSkinQuestionDto: UpdateSkinQuestionDto): Promise<SkinQuestion> {
  ...
  async removeQuestion(id: string): Promise<SkinQuestion> {
    const deletedQuestion = await this.skinQuestionModel.findByIdAndDelete(id).exec();
    if (!deletedQuestion) {
      throw new NotFoundException(`Skin question with ID ${id} not found`);
    }
  ...
  async submitSkinProfile(userId: string, submitDto: SubmitSkinProfileDto): Promise<SkinProfile> {
    // Validate that all questions exist
    const questionIds = submitDto.answers.map(answer => answer.questionId);
    const foundQuestions = await this.skinQuestionModel.find({ _id: { $in: questionIds } }).exec();
    ...
    const missingRequiredIds = requiredIds.filter(id => !answeredIds.includes(id));
    if (missingRequiredIds.length > 0) {
      throw new BadRequestException(`Missing answers for required questions: ${missingRequiredIds.join(', ')}`);
    }
  ...
  async deleteUserSkinProfile(userId: string): Promise<SkinProfile> {
    const deletedProfile = await this.skinProfileModel.findOneAndDelete({ userId }).exec();
